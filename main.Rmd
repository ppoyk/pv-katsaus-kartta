...
# (Ensin määritetään käytettävät värit)
palette <- c("1"="red", "2"="brown4", "3"="darkgoldenrod",
...
kartat_plot <- function(lon, lat, value, col) {
  ...
  geom_text_repel(data = input,
                  aes(x = lon, y = lat, colour = as.factor(col)),
                  label = value,
...
# Plottauksen värien rajojen laskenta (putkikohtaisesti)
df[["y_ka_50p"]] <- df$ref_ka + 0.5 * (df$ref_max - df$ref_ka) 
df[["y_ka_10p"]] <- df$ref_ka + 0.1 * (df$ref_max - df$ref_ka) 
df[["a_ka_50p"]] <- df$ref_ka + 0.5 * (df$ref_min - df$ref_ka) 
df[["a_ka_10p"]] <- df$ref_ka + 0.1 * (df$ref_min - df$ref_ka)

# Ajetaan värit sen mukaan, mihin rejojen väliin nykyinen korkeusarvo osuu 
df[["color"]] <- NA # Alustetaan sarake väreille
...
plot <- kartat_plot(df$KoordErLong, df$KoordErLat, df$ref_ka_delta, df$color)
...
plot <- kartat_plot(df_suuri$KoordErLong, df_suuri$KoordErLat,
                    df_suuri$ref_ka_delta, df_suuri$color)
...
plot <- kartat_plot(df_keskikoko$KoordErLong, df_keskikoko$KoordErLat,
                    df_keskikoko$ref_ka_delta, df_keskikoko$color)
...
plot <- kartat_plot(df_pieni$KoordErLong, df_pieni$KoordErLat,
                    df_pieni$ref_ka_delta, df_pieni$color)
...
high_cm    <- 30; low_cm    <- -30
above_cm <- 10; below_cm <- -10

df[["color_cm"]] <- NA # Alustetaan sarake väreille
...
plot <- kartat_plot(df$KoordErLong, df$KoordErLat, df$ref_ka_delta, df$color_cm)
