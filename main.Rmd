...
ENSIMMÄISTÄ KERTAA KÄYNNISTETTÄESSÄ SYÖTÄ renv::restore() KONSOLIIN, jos renv ei aktivoidu muuten !
...
list.files(file.path(D$koodi, "funktiot"), full.names = TRUE) |>
  sapply(source) |> invisible() # Lataa funktiot

# Tarkistetaan annetut kansiot
capture.output(lapply(D, check_dir, tilde = root, ensure = T)) |> invisible()
...
hist(jakauma, breaks=50); abline(v=log10(c(pien_kesk_lim, kesk_suur_lim)), lwd=2)
...
koonti <- data.frame()
save_path_pv <- file.path(D$output, "kuvaajat")
...
koonti <- data.frame(
  matrix(
    NA,
    ncol = length(names(pv_funktio(ltaulu[1,1],ltaulu[1,2],ajopvm,c(1990,2020),tempdir()))),
    nrow = nrow(ltaulu)
    )
  )
...
for (i in 1:nrow(ltaulu)) {
  ...
  tulos <- pv_funktio(m_id, a_id, latest_date = ajopvm,
                      ref_vuosi_vali = c(1960,2023),
...
koonti <- koonti[order(koonti$Tunnus_a), ]
koonti$X <- NULL
